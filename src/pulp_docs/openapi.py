...
class HostOpenAPIGenerator:
    """Generate openapi schemas using uv run on the host (no container required).
    ...
    def generate(self) -> dict[str, Path]:
        """Generate openapi json files. Return map of plugin labels to generated spec path."""
        if not self.plugins:
            return {}
        output_dir = Path(tempfile.TemporaryDirectory(delete=False).name)
