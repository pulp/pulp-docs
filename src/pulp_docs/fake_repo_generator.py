...
ContentType = t.Union[t.Literal["learn"], t.Literal["guide"],
                      t.Literal["reference"], t.Literal["tutorial"]]
...
class ContentFactory:
    ...
    def create_guide(self, title: t.Optional[str] = None, repository: t.Optional[str] = None, persona: t.Optional[str] = None) -> Document:
        """Creates a guide"""
        verbs = ["do", "create", "generate", "bind", "delete", "update"]

        def gen_paragraph():
            return " ".join(
                (self.faker.paragraph()) for _ in range(3))
        ...
        template_vars = {
            ...
            "steps": steps,
            "sections": [("{} - {}".format(i + 1, step), gen_paragraph())
                         for i, step in enumerate(steps)]
        }
        relative_path = "{}/guides".format(persona)
        return Document(title, "guide_page.md.j2", template_vars, relative_path=relative_path, basepath=self.basepath)

    def create_learn(self, title: t.Optional[str] = None, repository: t.Optional[str] = None, persona: t.Optional[str] = None) -> Document:
        """Creates a learn"""
        subject = ["database", "model", "content", "artifact", "repository"]
        subject_complement = ["configuration",
                              "deployment", "ecossystem", "setup"]
        adjective = ["resilient", "complex", "flexible",
        ...
        def gen_title():
            return " ".join([self.faker.word(ext_word_list=introductory),
        ...
        return Document(title, "learn_page.md.j2", template_vars, relative_path=relative_path, basepath=self.basepath)

    def create_tutorial(self, title: t.Optional[str] = None, repository: t.Optional[str] = None, persona: t.Optional[str] = None) -> t.List[Document]:
        """Creates a tutorial"""
        documents = []
        subject = ["the database", "the model",
                   "the content", "the artifact", "the repository"]
        verb = ["configuring", "deploying",
                "setting", "organizing", "doing", "handling"]
        adjective = ["infrastructure", "internals", "contexts",
                     "dependencie", "relationships", "signals"]
        topics = ["Content Management", "Development", "System Administration"]

        def gen_paragraphs(n: t.Optional[int] = None):
            n = n or 3
            return " ".join(
                (self.faker.paragraph()) for _ in range(n))

        def gen_title():
            return "Getting started with {}".format(self.faker.word(ext_word_list=topics))
        ...
        documents.append(
            Document(main_title, "tutorial_main.md.j2", template_vars, relative_path=relative_path, basepath=self.basepath))
        ...
        for i in range(randint(2, 4)):
            ...
            documents.append(
                Document(section_title, "tutorial_main.md.j2", template_vars, relative_path=relative_path, basepath=self.basepath))
...
class DocumentationFactory:
    ...
    def create_documentation(self) -> t.List[Document]:
        ...
        for persona in ("developer", "content-manager", "sys-admin"):
            [content_factory.create_guide(persona=persona).create(
                write=True) for _ in range(randint(1, 5))]
            [content_factory.create_learn(persona=persona).create(
                write=True) for _ in range(randint(1, 5))]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        "docs_generator", description="Genreate a doc structure populated with fake data")
    parser.add_argument(
    ...
    if basepath.exists():
        overwrite = input(
            f"{basepath} exists, do you wanna override? (N/y): ").lower() in ("y", "yes")
        if overwrite:
